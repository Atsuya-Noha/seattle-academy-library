...
public class BooksService {
	...
	public void updateBook(BookDetailsInfo bookInfo) {
		...
		if (bookInfo.getThumbnailUrl() == null) {
			...
			sql = "UPDATE books SET title = ?, author = ?, publisher = ?, Publish_date = ?, isbn = ?, description = ? WHERE bookId";
			...
			// TODO 取得した書籍情報を更新するようにSQLを修正（タスク５）
			sql = "UPDATE books SET title = ?, author = ?, publisher = ?, Publish_date = ?,thumbnail_name, thumbnail_url, isbn = ?, description = ? WHERE bookId";
			jdbcTemplate.update(sql, bookInfo.getTitle(), bookInfo.getAuthor(), bookInfo.getPublisher(),
